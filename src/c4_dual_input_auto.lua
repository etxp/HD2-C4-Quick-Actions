...
local M = {version='0.6.0-exp06', capture=true, tick=0, elapsed_ms=0,
...
local function emit(kind, extra)
    ...
    local row = {schema=1, record_type=kind, evidence_kind='RUNTIME_OBSERVATION',
        ...
        clock_kind='sum_of_valid_update_dt', phase=M.phase or 'initialization',
    ...
    local line = json(row)..'\n'
    assert(#line<1024*1024,'log_record_limit')
...
local ok,why=pcall(function()
    ...
    emit('layout_verified',{module_sha256=ActionLayout.module_sha256,
        action_execution_enabled=true,direct_mode_writes=false,mouse_mapping='RMB_DEPLOY_LMB_DETONATE',gamepad_mapping='LT_L2_DEPLOY_RT_R2_DETONATE'})
    cancel_keys={}
...
print('[C4 Dual Input EXP06] Automatic C4; RMB/LT/L2 Deploy; LMB/RT/R2 Detonate; F7 marker. Log: '..M.log_name)
return M
